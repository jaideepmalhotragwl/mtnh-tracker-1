...
export default function UploadModal({ onImport, onClose }) {
  ...
  function processFile(file) {
    ...
    reader.onload = e => {
      ...
      setPending(mapped)
      setPreview({ name: file.name, count: mapped.length, sample: mapped.slice(0, 4).map(p => p.site_name).filter(Boolean) })
    }
    ...
    <div className={styles.modal} style={{ maxWidth: 520 }}>
      ...
      <div className={styles.body}>
        <div className={styles.infoBox}>
          ...
          <p className={styles.infoText}>Site ID · Site Name · Zone · City · Circle · Site Type · Toco · Layer · Team Name · Installation Date · Billing Status · Remarks</p>
        </div>
        ...
        {preview && (
          <div className={styles.previewBox}>
            ...
            <div className={styles.previewSub}>{preview.sample.join(', ')}{preview.count > 4 ? '…' : ''}</div>
          </div>
        )}
      </div>
      <div className={styles.footer}>
        <button className={styles.cancelBtn} onClick={onClose}>Cancel</button>
        {preview && <button className={styles.primaryBtn} onClick={() => { onImport(pending); onClose() }}>Import {preview.count} Sites → Supabase</button>}
      </div>
    </div>
  )
}
